...
void SceneManager::fireProjectile()
{
	//static


	Camera* projectileCam = GraphicsPipeline::Instance()->GetCamera();

	Matrix4 projectileCrosshair = projectileCam->BuildViewMatrix();
	Vector3 projectileDirection = Matrix3::Transpose(Matrix3(projectileCrosshair))* Vector3(0, 0, -1);

	GameObject* sphereProjectile = CommonUtils::BuildSphereObject("bullet", projectileCam->GetPosition(), (0.25f), true, 1.5f, true, false, Vector4(1.0f, 1.0f, 1.0f, 1.0f));

	sphereProjectile->Physics()->SetLinearVelocity(projectileDirection * 20);

	

	scene->AddGameObject(sphereProjectile);
