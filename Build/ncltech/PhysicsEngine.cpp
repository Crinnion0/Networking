...
void PhysicsEngine::UpdatePhysics()
{
	...
	if (OctRoot != NULL) {
		...
		//buildOctree(Vector3(20.0f, 20.0f, 20.0f), Vector3(0, 0, 0), physicsNodes);
	}
...

			{
				...
				for (size_t j = i + 1; j < physicsNodes.size(); ++j)
				{
						...
						&& pnodeB->GetCollisionShape() != NULL)
					{
						...
						if (abs((pnodeA->GetPosition() - pnodeB->GetPosition()).Length()) <
							pnodeA->GetCollisionShape()->GetSphereRadius() + pnodeB->GetCollisionShape()->GetSphereRadius()) {
							CollisionPair cp;
							cp.pObjectA = pnodeA;
							cp.pObjectB = pnodeB;
