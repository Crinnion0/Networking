...
public:
	...
	inline virtual float GetSphereRadius() override {

		float holder = max(halfDims.x, halfDims.y);
		holder = max(holder, halfDims.z);
		return sqrt((holder * holder) + (holder * holder));
