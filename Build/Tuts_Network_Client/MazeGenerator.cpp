...
void MazeGenerator::Generate(initialiseData data)
{
	if (allNodes)
	{
	delete[] allNodes;
	...
	Generate_Sparse(data.maze_density);
...
vector<int> MazeGenerator::findWalls()
{

	vector<int> wallIndexs;

	for (uint i = 0; i < ((size*(size - 1)) * 2); i++) {
		if (allEdges[i]._iswall == true) {
			wallIndexs.push_back(i);
		}
	}
